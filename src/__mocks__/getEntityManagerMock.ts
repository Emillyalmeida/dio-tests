...
const getEntityManagerMock = async ({
  ...
  manager.update = jest
    .fn()
    .mockImplementation(() => Promise.resolve(updateReturn));
  manager.increment = jest
    .fn()
    .mockImplementation(() => Promise.resolve(incrementReturn));
  manager.decrement = jest
    .fn()
    .mockImplementation(() => Promise.resolve(decrementReturn));
  manager.count = jest
    .fn()
    .mockImplementation(() => Promise.resolve(countReturn));
  manager.delete = jest
    .fn()
